...
def detect_square(frame, model, last_count=-1, target_reached=False):
    ...
    for result in results:
        ...
        for box in result.boxes:
            # Seleziono quelli dei quadrati con confidenza maggiore di 0.6 
            if box.conf[0] > 0.6 and int(box.cls[0]) == 1:
    ...
    if squares_found > 0 and squares_found != last_count and not target_reached:
        print(f"\n{'='*60}")
        print(f"[DETECTION REPORT] Quadrati rilevati: {squares_found}")
        print(f"{'='*60}")
    ...
    color = (0, 255, 0) if squares_found < 4 else (0, 255, 255)  # Giallo quando raggiunge 4
    cv2.putText(frame, f"Squares: {squares_found}/4", (10, 30), 
                cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)

    return frame, squares_found
...
def run():
    ...
    while True:
        ...
        square_frame, squares_found = detect_square(frame, model_shapes, last_count, target_reached)
        cv2.imshow("Square Detection", square_frame)
        
        # Aggiorna il conteggio e controlla se abbiamo raggiunto 4
        if squares_found != last_count:
            last_count = squares_found
            if squares_found == 4 and not target_reached:
                target_reached = True
                print(f"\n{'🎯'*30}")
                print("TARGET RAGGIUNTO! 4 quadrati rilevati!")
                print(f"{'🎯'*30}\n")
